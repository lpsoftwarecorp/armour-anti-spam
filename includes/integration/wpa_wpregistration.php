...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	function wpa_wpregistration_extra_validation( $errors, $sanitized_user_login, $user_email ) {
		...
		if (wpa_check_is_spam($_POST)){ // phpcs:ignore WordPress.Security.NonceVerification.Missing -- reading third-party form data; spam trap cannot nonce-verify a form it does not own
			do_action('wpa_handle_spammers','wpregistration', $_POST); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- reading third-party form data; spam trap cannot nonce-verify a form it does not own

			if ( !is_object( $errors ) ) {	$errors = new WP_Error(); }

			$errors->add( 'wpa_extra_email', esc_html( $GLOBALS['wpa_error_message'] ) );
		}
		return $errors;
	}
