var wpa_field_name, wpa_unique_id, wpa_add_test, wpa_token, wpa_require_interaction, wpa_hidden_field;

document.addEventListener('DOMContentLoaded', function() {
    wpa_field_name = wpa_field_info.wpa_field_name;
    wpa_unique_id = wpa_field_info.wpa_field_value;
    wpa_add_test = wpa_field_info.wpa_add_test;
    wpa_token = wpa_field_info.wpa_token;
    wpa_require_interaction = wpa_field_info.wpa_require_interaction;

    wpa_hidden_field            = "<div id='altEmail_container' class='altEmail_container'><label for='alt_s'>Alternative:</label><input type='text' id='alt_s' name='alt_s' ></div><span class='wpa_hidden_field' style='display:none;height:0;width:0;'><label>WPA <input type='text' name='"+wpa_field_name+"' value='"+wpa_unique_id+"' /></label><input type='hidden' name='alt_t' value='"+wpa_token+"' /></span>";
    ...
    // Skipped when human interaction is required, so the trap field is only added
    // after a genuine interaction event (blocks JS bots that never interact).
    if (wpa_require_interaction !== 'yes') {
...
function wpa_add_trap_field() {
    ...
    if (typeof fluent_forms_global_var_1 !== 'undefined') { // QUICK HACK FOR FLUENT FORMS CONVERSIONAL
        ...
        fluent_forms_global_var_1.extra_inputs['alt_t'] = wpa_token;
    }
    // EOF FLUENT FORMS
...
function wpa_add_test_block() {
    const checkingTest = '<div class="wpa-test-msg"><strong>Armour Anti-Spam ( Only visible to site administrators. Not visible to other users. )</strong><br />This form has a spam trap enabled. If you want to act as spam bot for testing purposes, please click the button below.<br/><span class="wpa-button" onclick="wpa_act_as_spam()" data-actiontype="remove">Act as Spam Bot</span></div>';
...
function wpa_remove_trap_field() {
    ...
    if (typeof fluent_forms_global_var_1 !== 'undefined') {
        ...
        delete fluent_forms_global_var_1.extra_inputs['alt_t'];
    }
}